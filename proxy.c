...
int main(int argc, char **argv)
{
    ...
    int listenfd, connfd, serverconnfd, reqPort, pathExists, len, intResponse, total;
    ...
    char portAndPath[MAXLINE];
    char strPort[MAXLINE];
    char specificPath[MAXLINE];
    char httpBuffer[MAXLINE];
    char hostWebAddress[MAXLINE];
    char httpRequest[MAXLINE];
    char response[MAXLINE];
    char fileBuffer[MAXLINE];
    char *token;

    /*Web Proxy is going to listen for requests*/
    ...
    while(1){
    	...
    	/*Parsing incoming user request*/
    	read(connfd, recvline, MAXLINE);
    	sscanf(recvline, "%s %s %s", requestType, fullPath, httpBuffer);
    	...
    	if(strstr(fullPath, ":") != NULL){
    		...
    		token = strtok(NULL, "");
    		strcpy(portAndPath, token);
    		if(strstr(portAndPath, "/") != NULL ){
    			strcpy(strPort, strtok(portAndPath, "/"));
    	...
    	if(pathExists == PATH){
    		sprintf(httpRequest, "%s /%s HTTP/1.1\r\nHost: %s\r\n\r\n", requestType, specificPath, hostWebAddress);
    	}else if(pathExists == NOPATH){
    		sprintf(httpRequest, "%s / HTTP/1.1\r\nHost: %s\r\n\r\n", requestType, hostWebAddress);
    	}
    	
    	//If this condition is true,it guarantees the fields are filled out
    	if((pathExists == PATH || pathExists == NOPATH) && 						strcmp(requestType, "GET") == 0){
    		printf("%s\n", httpRequest);
    		fflush(stdout);
    	        rio_t robust;
